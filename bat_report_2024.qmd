...
In 2024, 35 acoustic recording units were placed across Edmonton and surrounding areas between July 15 and August 26. These recorders captured the high-frequency sounds bats use to navigate and find food. Monitoring followed guidelines from the North American Bat Monitoring Program (NABat) and the North by Northwest Bat Hub (@reichert2018guide).

##Data Processing
The recordings were first analyzed using two computer programs, Kaleidoscope and SonoBat, which automatically identify bat calls based on their acoustic characteristics. These programs helped narrow down the recordings that were most likely to contain calls from different bat species.
Recordings identified by either program were then reviewed manually. Because some bat species have very similar calls, this additional step is important for confirming species identifications. For each monitoring site and night, recordings were reviewed until at least one call from each detected species could be confidently identified.
Species identifications were checked using published reference information on bat calls (@szewczak2022echolocation; @VesperBats2022) and followed NABat review guidelines. The results were then summarized for each location and night.
Some recordings could be confidently identified to species, while others could only be assigned to a group of species with similar calls or identified simply as bats. Confirmed species identifications were used to summarize the number of species recorded at each location. All bat detections, including those that could not be identified to a single species, were retained when summarizing overall bat activity.
All recordings and their associated identifications are stored in WildTrax under the Bat Community - City of Edmonton project.
